...
| [review](./plugins/hank-dev/skills/review) | `/hank-dev:review` | Multi-agent review with a guarded DeepSeek pass. External review requires explicit per-run consent, a sensitive-content gate, an isolated temporary directory, exact single-file read permission, no automatic approval, and explicit failure reporting. |
...
| [mermaid-skill](./mermaid-skill) | Mermaid 图表生成（Mermaid diagrams）. 支持 flowchart、sequence、class、ER、state、Gantt、C4、architecture、mindmap 与 user journey 等图表。默认使用本地 `mmdc` 进行校验与导出，`Kroki` 仅在用户明确授权后作为远端兜底。 |
...
| [douyin-archive](./douyin-archive) | Archives Douyin (TikTok-CN) videos into a searchable corpus — one folder per video (mp4 + timestamped subtitle.srt + spoken-transcript.txt + meta) plus an INDEX retrieval map. yt-dlp can't download Douyin (a_bogus anti-crawl, extractor long-broken) → drives TikTokDownloader/DouK Web API; no subtitle track → ffmpeg + mlx-whisper ASR; foreign IP is geo-blocked → routes via an `ssh -D` mainland-China proxy. One command: tunnel → DouK API → browser cookie → download → transcribe → meta → index. |
| [opencode-deepseek-delegate](./opencode-deepseek-delegate) | Delegates a pure-text subtask to DeepSeek from an empty temporary directory with all tools denied. Local or private content requires explicit per-run consent. Nonzero exit, timeout, permission denial, fallback, error events, parse failure, and empty output are reported as failures. |
| [deepseek-developer](./deepseek-developer) | Produces a tightly constrained DeepSeek code patch only after per-run approval. Exact allowed files deterministically form the outbound request, sensitive paths and likely credentials are rejected, and only a strictly validated unified diff may proceed to Codex review. |
