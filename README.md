...
| [opencode-deepseek-delegate](./opencode-deepseek-delegate) | Delegates a scoped subtask to a DeepSeek model via the `opencode` CLI (`opencode run --agent deepseek-worker --auto --format json`) and returns plain text, for use inside `/team`/`/omc-teams`-style multi-agent orchestration where OMC's native worker types (`claude/codex/gemini/antigravity/grok/cursor`) are a closed enum and adding a 7th native type would mean forking OMC's plugin source. Documents three hard-won pitfalls (stdin EOF hang, missing `--auto` hang, mixed JSON/non-JSON stdout) and the primary-vs-subagent `opencode agent` mode distinction. |
