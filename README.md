...
开发存储磁盘清理（与 Mac Cleanup 配套：本技能管开发存储，mac-cleanup 管系统层应用）。四阶段流程：分层调研定位大头（`.claude/worktrees` 堆积 / Docker 构建缓存 / 开发缓存 / 用户媒体）→ 逐类安全核查（worktree 未提交/未推送检查、孤儿目录内容核实、temp 仓库先归档远程再删、用户创作内容保留）→ 分类清理 → df 验证释放效果。内含实战踩坑：孤儿 worktree 目录的 git 读数会 fallback 到主仓撒谎、squash 合并下 merge-base 判合并不可靠、Docker.raw 稀疏文件回收机制。实测单次释放 62G 零数据丢失。
