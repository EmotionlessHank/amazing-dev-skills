...
5. **Export** — use local `mmdc` to produce PNG/SVG/PDF. Use Kroki only after explicit user approval.
...
# Validate with Kroki only after explicit user approval
...
| `mmdc` error `Could not find Chrome` | Install the headless browser: `npx puppeteer browsers install chrome-headless-shell`, or request approval before using Kroki. |
| Kroki PDF fails with HTTP 400 | Kroki does PNG/SVG only for Mermaid; use local `mmdc` for PDF |
| Valid diagram reported "invalid" by `mmdc` | The error is a Chrome/puppeteer setup failure, not a syntax error. Do not rewrite correct `.mmd`; fix the browser, or request approval before validating with Kroki. |
