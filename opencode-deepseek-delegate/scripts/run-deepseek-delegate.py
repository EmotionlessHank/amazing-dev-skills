#!/usr/bin/env python3
"""以无工具权限执行纯文本 DeepSeek 委派，并严格解析 OpenCode JSONL。"""
    ...
    return {
        ...
        "agent": {
            "deepseek-worker": {
                "mode": "primary",
                "model": model,
                "permission": CONFIG["agent"]["deepseek-worker"]["permission"],
            }
        },
        "plugin": [],
        "instructions": [],
    ...
    child_env.update(
        {
            ...
            "OPENCODE_CONFIG_CONTENT": json.dumps(
                _load_isolated_config(), ensure_ascii=False, separators=(",", ":")
            ),
        }
    )
    return child_env
...
def main() -> int:
    ...
    try:
        if args.parse_fixture:
            ...
            result = run_delegate(args.prompt or "")
